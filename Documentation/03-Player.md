...
- **`OnPlayerDeath`** — a **static** event, which is worth pausing on: because there's only ever one player, this event doesn't need an instance to subscribe to (`Player.OnPlayerDeath += ...` rather than `somePlayerInstance.OnPlayerDeath += ...`). `Enemy.cs` subscribes to this in its own `OnEnable()`/unsubscribes in `OnDisable()` to know "the player just died, stop fighting" — covered in `04-Enemy.md`.
...
```
Same overlap-check idea as `Entity_Combat.PreformAttack()`, but note this call to `OverlapCircleAll` **doesn't pass a `LayerMask`** — it checks every collider in range regardless of physics layer, and relies entirely on "does this have an `IInteractable`?" as the filter instead. Finds the *closest* interactable within 1.5 units (not just the first one found) and calls `Interact()` on it alone — so standing between two NPCs only interacts with the nearer one.
...
```
Called from `Enemy_Health.Die()` (see `04-Enemy.md`) with that enemy's own `questTargetID` — so killing an enemy automatically advances *every* active quest that happens to target that same ID string, without the enemy needing to know anything about quests beyond carrying that one ID field. `AcceptQuest`/`QuestIsActive` round out the API (wrap a `Quest_DataSO` into runtime `QuestData` on accept; check by reference via `List.Find` with a lambda) — full detail belongs to a later Quest-focused doc, included here only because `Player_QuestManager` itself lives in the `Player/` folder.
...
```
Reuses `GetDetectedColliders()` (declared `protected` in `Entity_Combat` specifically so this subclass could reuse the same overlap-circle logic instead of duplicating it) but checks for `ICounterable` (from `01-Foundations.md`) instead of `IDamagable` — same "overlap check, filter by interface" shape as `PreformAttack`, applied to a different contract. `Player_CounterAttackState` (below) is what actually calls this.
